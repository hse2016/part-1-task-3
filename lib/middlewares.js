...
const Base64EncoderTransformStream = require('./encoder.js').Base64EncoderTransformStream;
...
let createFileSeekerMiddleware = function(useBase64) {
  return function(req, res, next) {
    ...
    fs.stat(fullpath, function(err, stats) {
      ...
      if (stats.isFile()) {
        ...
        if (useBase64) {
          transformerStream = new Base64EncoderTransformStream();
        } else {
          transformerStream = new TransformTransliterator();
        }
